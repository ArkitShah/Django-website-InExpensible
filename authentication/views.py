        ...
        if not User.objects.filter(username = username).exists():
            if not User.objects.filter(email = email).exists():
                if len(password) < 8:
                    ...
                    messages.info(request, 'Please check your email for activation link.')
                    return render(request, 'authentication/register.html')
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if user.is_active:
                ...
                user.is_active = True
                user.save()
                messages.success(request, 'Your account has been activated! Login to continue.')
                return redirect('login')
...
class CompletePasswordReset(View):
    def get(self, request, uidb64, token):
        ...
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                messages.warning(request, 'This link has already been used once!')
                return render(request, 'authentication/reset-password.html')
    ...
    def post(self, request, uidb64, token):
        ...
        if len(password) < 8:
            messages.error(request, "Password too short! It must be atleast 8 characters long.")
            return render(request, 'authentication/set-newpassword.html', context)
        try:
            user_id = force_text(urlsafe_base64_decode(uidb64))
            user = User.objects.get(pk=user_id)
            user.set_password(password)
            user.save()
            messages.success(request, 'Password reset successfully! You can login with the new password now.')
            return redirect('login')
        except Exception as identifier:
            messages.info(request, 'Something has gone wrong! Please try again.')
            return render(request, 'authentication/set-newpassword.html', context)